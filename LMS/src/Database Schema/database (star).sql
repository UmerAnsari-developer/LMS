...
(
    ...
    FILENAME = N'C:\SQLData\LibraryManagement_System_Data.mdf',
    SIZE = 512MB,
    MAXSIZE = UNLIMITED,
    FILEGROWTH = 128MB
)
...
(
    ...
    FILENAME = N'C:\SQLLog\LibraryManagement_System_Log.ldf',
    SIZE = 256MB,
    MAXSIZE = 2GB,
    FILEGROWTH = 64MB
);
...
ON dw.fact_book_issue (issue_id, student_key, book_key, issue_date_key, days_late);
GO
...
ALTER TABLE oltp.book_info ADD CONSTRAINT DF_book_qty DEFAULT 1 FOR book_quantity;
...
BEGIN
    ...
    BEGIN
        ...
        SELECT admin_id, name, @p_deleted_by, @p_deleted_role, @p_reason
    ...
    BEGIN
        INSERT INTO oltp.deleted_student (student_id, name, deleted_by, role, reason)
        SELECT librarian_id, name, @p_deleted_by, @p_deleted_role, @p_reason
    ...
    BEGIN
        ...
        SET @deleted_role = @p_deleted_role;
        SET @deleted_reason = @p_reason;
...
BEGIN
    ...
    SELECT copy_id, @v_title, @v_author, @v_isbn, @p_deleted_by, @p_deleted_role
    FROM oltp.book_copy WHERE book_id = @p_book_id;
...
-- ==================================================================
-- SECTION 12: VIEWS (Reporting & BI Layer)
-- ==================================================================
...
SELECT
    ...
    SUM(CASE WHEN s.restriction_status = 'Restricted' THEN 1 ELSE 0 END) AS restricted_students,
    SUM(CASE WHEN s.student_id IN (
        SELECT ib.student_id FROM oltp.issued_book ib WHERE ib.is_returned = 0
    ) THEN 1 ELSE 0 END) AS students_with_books,
    SUM(
        (SELECT ISNULL(SUM(fine_amount), 0)
         FROM oltp.issue_fine f
         WHERE f.student_id = s.student_id AND f.payment_status = 'UNPAID')
    ) AS total_unpaid_fines_dept
